...
pub async fn server_mode(args: ServerArgs) -> std::io::Result<()> {
    ...
    if web_config.enabled {
        let web_bind = web_config.bind.clone();
        let web_port = web_config.port;
        let web_path = web_config.path.clone();

        info!(
            "Starting Web UI server on {}:{} serving files from: {}",
            web_bind, web_port, web_path
        );

        let web_server = HttpServer::new(move || {
            App::new().service(fs::Files::new("/", web_path.clone()).index_file("index.html"))
        })
